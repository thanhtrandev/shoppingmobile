...
begin
 ...
 SELECT p.product_id ,p.product_name, p.product_des, p.product_price, p.product_img_source, p.product_type, p.product_brand FROM ( 
  SELECT *, ROW_NUMBER() OVER (ORDER BY product_id) as rowc 
  FROM Products p where p.product_type = p_b) p WHERE p.rowc >= p_u and p.rowc <= p_v;
end//
...
begin
 ...
 SELECT p.product_id ,p.product_name, p.product_des, p.product_price, p.product_img_source, p.product_type, p.product_brand FROM ( 
  SELECT *, ROW_NUMBER() OVER (ORDER BY product_id) as rowc 
  FROM Products p
 ) p WHERE p.rowc >= p_u and p.rowc <= p_v;
end//
